...
int search_for_trails1(TrailData origin,
    ...
    for(size_t r = 0; r < topoMap.size(); r++){
        for(size_t c = 0; c < topoMap[r].size(); c++){
            ...
            if(topoMap[r][c] == 0){
                ...
                trailheadScore += search_for_trails1(origin, topoMap, topoMap.size(), 
                                                    topoMap[r].size());
            }
        }
    }
...
int search_for_trails2(TrailData origin,
        const std::vector<std::vector<int>>& topoMap, int mapRows,
        int mapCols)
{
    // neighbors: up, down, left, right
...
int part02(const std::vector<std::vector<int>>& topoMap)
{
    int trailheadScore = 0;
    ...
    for(size_t r = 0; r < topoMap.size(); r++){
        for(size_t c = 0; c < topoMap[r].size(); c++){
            ...
            if(topoMap[r][c] == 0){
                TrailData origin(topoMap[r][c], r, c);
                trailheadScore += search_for_trails2(origin, topoMap, topoMap.size(), 
                                                    topoMap[r].size());
            }
        }
    }
